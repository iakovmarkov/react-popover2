...
var pickZone = function pickZone(opts, frameBounds, targetBounds, size) {
  var t = targetBounds;
  var f = frameBounds;
  var zones = [{ side: 'end', standing: 'below', flow: 'column', order: 1, w: f.x2, h: f.y2 - t.y2 }, { side: 'end', standing: 'right', flow: 'row', order: 1, w: f.x2 - t.x2, h: f.y2 }, { side: 'start', standing: 'above', flow: 'column', order: -1, w: f.x2, h: t.y }, { side: 'start', standing: 'left', flow: 'row', order: -1, w: t.x, h: f.y2 }];
